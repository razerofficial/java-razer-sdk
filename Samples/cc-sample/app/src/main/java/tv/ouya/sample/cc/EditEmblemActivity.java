...
public class EditEmblemActivity extends Activity implements AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().hasExtra(EXTRA_MOD_UUID)) {
            ...
            if(gameModManager.isInitialized()) {
                ...
                gameModManager.registerInitializedListener(new GameModManager.InitializedListener() {
                    @Override
    ...
    private void loadEmblemFromMod() {
        mMod = GameModManager.getInstance().getLocal(mModUuid);
    ...
    private void saveEmblem() {
        if(mMod == null) {
            mMod = GameModManager.getInstance().create();
        ...
        try {
            GameMod.Editor e = mMod.edit();

            // Clear out old data
            for(String s : mMod.getFilenames()) {
                e.deleteFile(s);
            }
            for(GameModScreenshot s : mMod.getScreenshots()) {
            ...
            // Save changes
            e.save(new GameModManager.SaveListener() {
                @Override
                ...
                @Override
                public void onError(GameMod gameMod, int errorCode, String message) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if(event.getAction() == KeyEvent.ACTION_DOWN) {
            Controller.onKeyDown(event.getKeyCode(), event);
        }
        if(event.getAction() == KeyEvent.ACTION_UP) {
            Controller.onKeyUp(event.getKeyCode(), event);
        }
    ...
    public boolean dispatchGenericMotionEvent(MotionEvent event) {
        Controller.onGenericMotionEvent(event);
