...
public class EmblemView extends SurfaceView implements View.OnClickListener{
    ...
    private void inputFrame() {
        ...
        final Controller c = Controller.getControllerByPlayer(0);
        if(c != null && isEditing()) {
            if(c.getButton(Controller.BUTTON_A) || c.getButton(Controller.BUTTON_O)) {
                mWantsToEdit = false;
                return;
            }

            if(c.getButton(Controller.BUTTON_DPAD_RIGHT)) {
                l.posX += MOVE_SPEED;
                mChangeMade = true;
            }
            if(c.getButton(Controller.BUTTON_DPAD_LEFT)) {
                l.posX -= MOVE_SPEED;
                mChangeMade = true;
            }
            if(c.getButton(Controller.BUTTON_DPAD_DOWN)) {
                l.posY += MOVE_SPEED;
                mChangeMade = true;
            }
            if(c.getButton(Controller.BUTTON_DPAD_UP)) {
                l.posY -= MOVE_SPEED;
                mChangeMade = true;
            }
            ...
            final float LS_X = c.getAxisValue(Controller.AXIS_LS_X);
            final float LS_Y = c.getAxisValue(Controller.AXIS_LS_Y);
            final float RS_X = c.getAxisValue(Controller.AXIS_RS_X);
            final float RS_Y = c.getAxisValue(Controller.AXIS_RS_Y);

            if(Math.abs(LS_X) > Controller.STICK_DEADZONE) {
                l.scaleX += LS_X * SCALE_SPEED;
                l.scaleX = clamp(l.scaleX, SCALE_MIN, SCALE_MAX);
                mChangeMade = true;
            }
            if(Math.abs(LS_Y) > Controller.STICK_DEADZONE) {
                l.scaleY -= LS_Y*SCALE_SPEED;
                l.scaleY = clamp(l.scaleY, SCALE_MIN, SCALE_MAX);
                mChangeMade = true;
            }

            if(Math.abs(RS_X) > Controller.STICK_DEADZONE) {
