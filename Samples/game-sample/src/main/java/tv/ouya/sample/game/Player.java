...
public class Player extends RenderObject {
    ...
    static public boolean isStickNotCentered(float axisX, float axisY) {
        ...
        return (stickMag >= Controller.STICK_DEADZONE);
    }

    private void checkPlaySound(Controller controller) {
        if (controller.buttonChangedThisFrame(Controller.BUTTON_Y)) {
            Log.d("BUTTON", "State changed");
        }
        if (controller.buttonPressedThisFrame(Controller.BUTTON_Y)) {
            int sfxIdx = 0;
    ...
    private void getForwardAmountFromController(Controller c) {
        float axisX = c.getAxisValue(Controller.AXIS_LS_X);
        axisX = Math.min(axisX, 1.0f);
        float axisY = c.getAxisValue(Controller.AXIS_LS_Y);
    ...
    private void getShootDirFromController(Controller controller) {
        float axisX = controller.getAxisValue(Controller.AXIS_RS_X);
        axisX = Math.min(axisX, 1.0f);
        float axisY = controller.getAxisValue(Controller.AXIS_RS_Y);
        axisY = Math.min(axisY, 1.0f);
        if (isStickNotCentered(axisX, axisY)) {
            float stickMag = stickMag(axisX, axisY);
            ...
            if (controller.getButton(Controller.BUTTON_O)
                   || controller.getButton(Controller.BUTTON_U)) {
    ...
    protected void update() {
        ...
        checkPlaySound(controller);
        getForwardAmountFromController(controller);
        getShootDirFromController(controller);

        if (!isDead && forwardAmount != 0.0f) {
            if (controller.getButton(KeyEvent.KEYCODE_BUTTON_L2) || controller.getButton(KeyEvent.KEYCODE_BUTTON_R2)) {
                forwardAmount *= -0.75f;
            }
