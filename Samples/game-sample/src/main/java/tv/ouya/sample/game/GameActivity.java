...
public class GameActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            Controller controller = Controller.getControllerByPlayer(i);
            if (controller != null) {
                findOrCreatePlayer(controller.getDeviceId());
            }
        }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        if (keyCode == Controller.BUTTON_MENU) {
            int playerNum = Controller.getPlayerNumByDeviceId(event.getDeviceId());
            String menuButtonName = Controller.getButtonData(Controller.BUTTON_MENU).buttonName;
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        Controller controller = Controller.getControllerByDeviceId(event.getDeviceId());
        if (controller != null) {
            if (Player.isStickNotCentered(
                    controller.getAxisValue(Controller.AXIS_LS_X),
                    controller.getAxisValue(Controller.AXIS_LS_Y))) {
    ...
    private Player findOrCreatePlayer(int deviceId) {
        int playerNum = Controller.getPlayerNumByDeviceId(deviceId);
        if (playerNum < 0) {
            return null;
        }
